                    ...
                    {blog.tags && blog.tags.length > 0 && (
                        <div className="flex flex-wrap justify-center gap-2">
                            {blog.tags.map((tag, index) => (
                                <span key={index} className="px-3 py-1 text-sm text-white bg-blue-500 rounded-full">{tag}</span>
                            ))}
                        </div>
                    )}

                    {/* Date */}
                    <p className="text-sm">Publish at: {new Date(blog.date).toLocaleDateString()}</p>
                </div>
